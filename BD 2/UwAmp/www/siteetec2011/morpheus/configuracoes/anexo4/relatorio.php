...
$codEtapa = mysql_result($rsEtapaAtual, 1, 0);
    ...
    if (isset($_GET["codTurma"])) {
      ...
      $sqlTurmas = "SELECT Turmas.*, Series.* FROM Turmas 
          INNER JOIN Series ON Series.codSerie=Turmas.codSerie
          WHERE status='Cursando' ORDER BY serie";
      ...
      <div class="navbar-inner">
       ...
       <?php
        while($rTurma = mysql_fetch_array($rsTurmas)){
          ...
          echo "<li $active><a href='relatorio.php?codTurma=$codTurmar' >$turma</a></li>";
        }
      ?>
        </ul>
      </div>
    ...
    <tbody>
      ...
      for ($i=2; $i < 8 ; $i ++ ){
        ...
        while ($rDisciplina = mysql_fetch_array($rsDisciplinas)) {
          ...
          echo "<td>$aulas_com_divisao</td>";
          echo "<td>$aulas_previstas</td>";
          echo "<td>$aulasDadas</td>";
          echo "<td>$def</td>";
          echo "<td>$reposicao_substituicao</td>";
        }
