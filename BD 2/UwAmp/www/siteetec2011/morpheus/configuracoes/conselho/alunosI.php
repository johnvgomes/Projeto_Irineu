...
?>	
	...
	function abrirConselho(){
		var row = $('#table_alunos').datagrid('getSelected');
		if (row){
			url = "intermediario/conselho_intermediario.php?codturma=<?php echo $codTurma ?>&nchamada="+row.nChamada;
		}else{
			url = "intermediario/conselho_intermediario.php?codturma=<?php echo $codTurma ?>&nchamada=1";
	...
	function abrir_ata(){
		window.open("ata/relatorio_ata.php?codTurma=<?php echo $codTurma ?>");		
	}
</script>
...
<div id='excel'></div>
	...
	<table id="table_alunos" title="Conselho Intermediário do <?php echo $turma ?>" class="easyui-datagrid" style="width:150x;height:500px"
			url="get_alunos.php?etapa=I&codTurma=<?php echo $codTurma?>"
			toolbar="#toolbar"
			rownumbers="false" 
			fitColumns="true" 
			singleSelect="true" 
