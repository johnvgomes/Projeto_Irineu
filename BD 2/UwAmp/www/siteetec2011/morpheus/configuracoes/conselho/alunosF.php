...
?>	
	...
	function abrirConselho(){
		var row = $('#table_alunos').datagrid('getSelected');
		if (row){
			url = "final/conselho_final.php?codturma=<?php echo $codTurma ?>&nchamada="+row.nChamada;
		}else{
			url = "final/conselho_final.php?codturma=<?php echo $codTurma ?>&nchamada=1";
	...
	function ficha_individual(){
		url = "final/ficha_individual.php?codturma=<?php echo $codTurma ?>";
		window.open(url);
	}
	...
	function abrir_ata(){
		window.open("ata/relatorio_ata_final.php?codTurma=<?php echo $codTurma ?>");		
	}
...
<div id='excel'></div>
	...
	<table id="table_alunos" title="Conselho Final do <?php echo $turma?>" class="easyui-datagrid" style="width:890px;height:500px"
			url="get_alunos.php?etapa=F&codTurma=<?php echo $codTurma?>"
			toolbar="#toolbar"
			rownumbers="false" 
			fitColumns="true" 
			singleSelect="true" 
			...
			<tr>
				...
				<th field="frequencia" width="20" align="center" formatter="formatFreq">% Freq.</th>
				<?php 
				while ($row = mysql_fetch_array($rsDisciplinas)){
					$codDisciplina = $row["codDisciplina"];
					$disciplina = $row["sigla"];
					echo "<th field='$codDisciplina' width='10' align='center' formatter='formatMencao'>$disciplina</th>";
				}
				?>
				<th field="PP1" width="30">PP1</th>
				<th field="PP2" width="30">PP2</th>
				<th field="PP3" width="30">PP3</th>
			</tr>
			</thead>
	</table>
	<div id="toolbar">
		<a href="#" class="easyui-linkbutton" iconCls="icon-edit" plain="true" onclick="editar()">Editar Decisões do Conselho</a>
		<a href="export/exportar.php?etapa=F&codturma=<?php echo $codTurma ?>" target="_blank" class="easyui-linkbutton" iconCls="icon-save" plain="true">Salvar como Planilha</a>
		<a href="#" class="easyui-linkbutton" iconCls="icon-redo" onclick="abrirConselho()" plain="true">Tela Cheia</a>
		<a href="#" class="easyui-linkbutton" iconCls="icon-print" plain="true" onclick="abrir_ata()">Ata do Conselho</a>
		<a href="#" class="easyui-linkbutton" iconCls="icon-print" plain="true" onclick="ficha_individual()">Ficha Individual</a>
	</div>
