...
while ($row = mysql_fetch_array($rsAlunos)) {
	...
	/* AQUELA FORÇADA BÁSICA FEITA 10 MINUTOS ANTES DO CONSELHO COMEÇAR
	   ...
	   if ( $codTurma == 84) $total_aulas_dadas = 740; //1C
	   if ( $codTurma == 87) $total_aulas_dadas = 780; //2C
	   if ( $codTurma == 85) $total_aulas_dadas = 800; //1D
	   if ( $codTurma == 88) $total_aulas_dadas = 820; //2D
