...
class ImagemNoticia {
    ...
    public function visualizarAdmin($id_noticia) {
        try {
            $sql = "SELECT * FROM 
            imagem_noticia WHERE id_noticia = ?";
            ...
            while ($linha = $sqlprep->fetch(PDO::FETCH_NUM)) {
                echo "<a href='?p=imagem_noticia/trocarimagem&img=$linha[2]&id=$linha[0]' title='$linha[2]'>
                        <img src='../imagem_noticia/$linha[2]' alt='$linha[2]'>
                    </a>
                ";
            }
            echo "</div>";
    ...
    public function editarImagem($id, $novo, $temporario, $anterior) {
        try {
            ...
            echo "Erro no editar imagem notícia " . $e->getMessage();
        }
    }
    ...
    public function excluir($id_noticia) {
        try {
            ...
            echo "Erro no excluir imagem notícia " . $e->getMessage();
        }
    }
