...
class Paginar{
    ...
    public function paginacao($numreg,$pg,$table,$pasta,$px,$link,$imagem,$titulo){
	...
	$sql = "SELECT * FROM $table ORDER BY id DESC LIMIT $inicial, $numreg";
        ...
        while($linha = $result->fetch(PDO::FETCH_NUM)) {
            ...
            "<tr>
                <td>
                    <img src='$pasta$linha[$imagem]' width='$px' />
                </td>  
                <td>
                    $linha[$titulo]
                </td>
                <td>
                <a href='".$this->url->getBase()."visualizar/".$linha[$titulo]."'>
                    &raquo; Clique aqui</a>
                </td>
            </tr>";
